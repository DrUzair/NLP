...
''' Look at all previous characters to generate next
    @Author: Uzair Ahmad
    2022
    +Transformer Blocks + Skip Connections (residual pathway) (line 23, 31, 48, 53)
'''
...
class TransformerBlockLanguageModel(nn.Module):
    class TransformerBlock(nn.Module):
        def __init__(self, head_count, embed_size):
            ...
            self.comm = TransformerBlockLanguageModel.MultiHeadAttention(head_count=head_count,
                                                                         in_size=embed_size,
                                                                         out_size=head_size)
            self.think = TransformerBlockLanguageModel.MLP(embed_size=embed_size)

        def forward(self, x):
            return x + self.think(x + self.comm(x))  # skip connections

    class MLP(nn.Module):
        # mlp : Attention is all you need : 3.3 Position-wise Feed-Forward Networks
    ...
    class MultiHeadAttention(nn.Module):
        ...
        def __init__(self, head_count, in_size, out_size):
            ...
            self.heads = nn.ModuleList(
                TransformerBlockLanguageModel.SelfAttentionHead(in_size, out_size // head_count)
                for _ in range(head_count)
            )
            # out_size*head_count is embed_size
            # project back into the residual pathway
            self.proj = nn.Linear(out_size * head_count, out_size * head_count)
    ...
    def forward(self, in_ids, target=None):
        if self.is_pos_emb:
            i = in_ids[:, -self.input_length:]
            in_ids_emb = self.token_embeddings_table(i)
            # Note: when generating next token, the input_length of in_ids may be different
            # take only 'as many as' in in_ids AND 'as max as' the input_length
            in_pos_emb = self.position_embeddings_table(
                torch.arange(in_ids[:, -self.input_length:].shape[1], device=self.device)
            )
            emb = in_ids_emb + in_pos_emb
            # Yes. it does mean that sa_head will take only as long context as input_length
        else:
            emb = self.token_embeddings_table(in_ids)  # batch_size x embed_size
    ...
    def generate(self, context_token_ids, max_new_tokens):
        for _ in range(max_new_tokens):
            # if self.is_pos_emb:
            #     #  for max_new_tokens > input_length,
            #     #  the index for pos_embeddings_table will go out of range
            #     context_token_ids = context_token_ids[:, -self.input_length:]
    ...
    def prep(self, corpus):
        ...
        self.blocks = nn.Sequential(
            TransformerBlockLanguageModel.TransformerBlock(head_count=self.sa_multihead_count,
...
text = 'a quick brown fox jumps over the lazy dog.\n ' \
       ...
       'a dog is lazy and fox jumps quickly.\n' \
       'fox jumps and dog is lazy.\n' \
       'dog is lazy and fox is brown who jumps over the dog.'

# text = 'go long ago longer a go go go go long ago longer go go go go long ago longer go go go'

model = TransformerBlockLanguageModel(batch_size=32,
                                      input_length=8,
                                      embed_size=32,
                                      sa_multihead_count=4,
                                      sa_head_size=32,
...
model.fit(train_iters=5000, eval_iters=1000, lr=1e-3)
