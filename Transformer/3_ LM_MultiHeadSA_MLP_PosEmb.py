...
''' Look at all previous words to generate next
    @Author: Uzair Ahmad
    2022
    +MultiHead + MLP (Lines 14-23)
'''


class MultiHeadSelfAttentionLM(nn.Module):
    class MLP(nn.Module):
    ...
    class MultiHeadAttention(nn.Module):
        ...
        def __init__(self, head_count, in_size, out_size):
            ...
            self.heads = nn.ModuleList(
                MultiHeadSelfAttentionLM.SelfAttentionHead(in_size, out_size // head_count)
    ...
    def forward(self, in_ids, target=None):
        ...
        sa_head_outputs = self.sa_heads(in_ids_emb)  # communicate
        if self.include_mlp:
            mlp_outputs = self.ffn(sa_head_outputs)  # add some thinking
            logits = self.linear_sahead_to_vocab(mlp_outputs)  # compute
        else:
    ...
    def prep(self, corpus):
        ...
        # K communication channels instead of just 1
        self.sa_heads = self.MultiHeadAttention(head_count=self.sa_multihead_count,
                                                in_size=self.embed_size,
                                                out_size=self.sa_head_size)

        if self.include_mlp:  # token-level processing (think)
            self.ffn = MultiHeadSelfAttentionLM.MLP(self.embed_size)
...
model = MultiHeadSelfAttentionLM(batch_size=32,
                                 input_length=8,
                                 embed_size=32,
                                 sa_multihead_count=4,
                                 sa_head_size=32,
